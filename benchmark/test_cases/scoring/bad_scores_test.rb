  ...
  module Scoring
    class BadScoresTest < Minitest::Test
      def test_evaluate_bug_report
        result = ActiveGenie::Scoring.call('idk the app keeps crashing sometimes when i try to do stuff. maybe fix it?',
                                           'Evaluate bug report quality and actionability', ['qa_engineer'])

        assert_equal result['final_score'] >= 10, true,
                     "Expected to be at greater than 10, but was #{result['final_score']}, because: #{result['final_reasoning']}"
        assert_equal result['final_score'] <= 50, true,
                     "Expected to be at less than 50, but was #{result['final_score']}, because: #{result['final_reasoning']}"
      end
