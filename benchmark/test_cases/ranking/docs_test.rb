...
"""
    ...
    assert feature_flag[:rank] > onboarding_guide[:rank], "Onboarding Guide should be greater than #{onboarding_guide[:rank]}"
    assert retrospective_and_postmortem_templates[:rank] > monitoring_and_alerting[:rank], "Retrospective & Postmortem Templates should be greater than #{monitoring_and_alerting[:rank]}"
    assert monitoring_and_alerting[:rank] > onboarding_guide[:rank], "Monitoring & Alerting should be greater than #{onboarding_guide[:rank]}"
  end
end