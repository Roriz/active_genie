...
module ActiveGenie
  module DataExtractor
    class OthersTest < Minitest::Test
      OTHERS = [
        { input: ['Roriz is 25 years old', { name: { type: 'string' }, age: { type: 'integer' } }],
          expected: { name: 'Roriz', age: 25 } },
        ...
        { input: ['Current temperature in New York is -5°C with 75% humidity and light snow',
                  { city: { type: 'string' }, temperature: { type: 'integer' }, humidity: { type: 'integer' },
        ...
        { input: ['Modern 3-bedroom apartment, 120m² with 2 bathrooms, located in Downtown. Monthly rent: $2,500',
                  { bedrooms: { type: 'integer' }, area: { type: 'number' }, bathrooms: { type: 'integer' },
                    location: { type: 'string' }, rent: { type: 'number' } }],
        ...
        { input: ['Senior Software Engineer position at Google - 5+ years experience required. Salary range: $150,000-$200,000/year',
                  { company: { type: 'string' }, position: { type: 'string' }, min_experience: { type: 'integer' },
                    min_salary: { type: 'integer' }, max_salary: { type: 'integer' } }],
          expected: { company: 'Google', position: 'Senior Software Engineer', min_experience: 5, min_salary: 150_000,
                      max_salary: 200_000 } },
        { input: ['Flight AA123 from LAX to JFK, departure at 10:30 AM, gate B12, duration: 5h 45m',
                  { flight_number: { type: 'string' }, origin: { type: 'string' }, destination: { type: 'string' },
                    gate: { type: 'string' }, duration_minutes: { type: 'integer' } }],
        ...
        { input: ['VIP ticket for Taylor Swift concert on March 15, 2024, at Madison Square Garden, Row A, Seat 12, Price: €250',
                  { event: { type: 'string' }, venue: { type: 'string' }, iso_date: { type: 'string' }, row: { type: 'string' },
                    seat: { type: 'integer' }, price: { type: 'number' }, currency_ISO_4217: { type: 'string' } }],
          expected: { event: 'Taylor Swift concert', venue: 'Madison Square Garden', iso_date: '2024-03-15', row: 'A',
                      seat: 12, price: 250, currency_ISO_4217: 'EUR' } },
        { input: ['2022 Tesla Model 3, Electric, 15,000 miles, Autopilot included. Listed at $45,995',
                  { year: { type: 'integer' }, make: { type: 'string' }, model: { type: 'string' }, mileage: { type: 'integer' },
                    price: { type: 'number' } }],
          expected: { year: 2022, make: 'Tesla', model: 'Model 3', mileage: 15_000, price: 45_995 } },
        { input: ['Add 250g of organic whole wheat flour and 2.5 tsp of active dry yeast',
                  { flour_amount: { type: 'number' }, flour_unit: { type: 'string' }, yeast_amount: { type: 'number' },
                    yeast_unit: { type: 'string' } }],
        ...
        { input: ['Final score: Lakers 112 - Warriors 108, MVP: LeBron James with 35 points and 12 assists',
                  { winner: { type: 'string' }, winner_score: { type: 'integer' }, loser: { type: 'string' },
                    loser_score: { type: 'integer' }, mvp: { type: 'string' }, mvp_points: { type: 'integer' }, mvp_assists: { type: 'integer' } }],
      ...
      OTHERS.each_with_index do |test, _index|
