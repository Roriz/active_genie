  ...
  module Ranker
    class McuTest < Minitest::Test
      def test_rank_strongest_character_from_mcu
        characters = JSON.parse(File.read(File.join(__dir__, '../assets/marvel_characters.json')),
        ...
        thanos_index = result.data.index { |r| r.include? 'Thanos' }
        tony_stark_index = result.data.index { |r| r.include? 'Tony Stark' }
        happy_hogan_index = result.data.index { |r| r.include? 'Happy Hogan' }

        assert_equal 51, result.data.length
